      ...
      <div className="flex flex-wrap gap-3">
        <div className="relative flex-1 min-w-[220px]">
          ...
          <Input
            ...
            className="pl-10 bg-[#202023] border border-zinc-800 text-white rounded-lg focus:ring-0 placeholder-zinc-500"
          />
        </div>
        <Select
          value={statusFilter}
          onChange={(e) => setStatusFilter(e.target.value as 'all' | InvoiceStatus)}
          className="w-auto bg-[#202023] border border-zinc-800 text-white rounded-lg"
        >
          <option value="all" className="bg-[#202023] text-white">All statuses</option>
          <option value="draft" className="bg-[#202023] text-white">Draft</option>
          <option value="sent" className="bg-[#202023] text-white">Sent</option>
          <option value="partial" className="bg-[#202023] text-white">Partial</option>
          <option value="paid" className="bg-[#202023] text-white">Paid</option>
          <option value="overdue" className="bg-[#202023] text-white">Overdue</option>
          <option value="void" className="bg-[#202023] text-white">Void</option>
        </Select>
      </div>
        ...
        <>
          <div className="md:hidden space-y-4">
            {Array.from({ length: 4 }).map((_, i) => (
              <div key={i} className="bg-[#202023] border border-zinc-800 p-4 rounded-xl h-32 animate-pulse" />
            ))}
          </div>
          ...
          <div className="md:hidden space-y-4">
            {pageItems.map((inv) => (
              <div
                key={inv.id}
                onClick={() => router.push(`/invoice/${inv.id}`)}
                className="bg-[#202023] p-4 rounded-xl border border-zinc-800/80 cursor-pointer hover:border-zinc-700 transition-colors relative pl-6"
              >
                <div
                ...
                <div className="flex justify-between items-start mb-3">
                  ...
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className={`text-xs capitalize font-medium ${
                      ...
                      inv.status === 'partial' ? 'text-purple-450' :
                      inv.status === 'draft' ? 'text-zinc-400' :
                      inv.status === 'overdue' ? 'text-amber-450' : 'text-red-450'
                    }`}>{inv.status}</span>
                  </div>
                </div>
                ...
                <div className="space-y-2 text-sm text-zinc-400">
                  <div className="flex justify-between">
                    <span className="flex items-center gap-2"><FiUser size={14} /> {inv.customers?.name || 'N/A'}</span>
                    <span className="font-mono font-semibold text-zinc-100">{formatCurrency(inv.total, inv.currency)}</span>
                  </div>
                  <div className="flex justify-between items-center text-xs">
                    <TypeBadge type={inv.type} />
                    <span className="flex items-center gap-1"><FiCalendar size={12} /> {new Date(inv.created_at).toLocaleDateString()}</span>
          ...
          <div className="hidden md:block bg-[#202023] border border-zinc-800 rounded-xl overflow-hidden shadow-sm">
            <table className="w-full text-sm">
              <thead className="bg-[#1A1A1C] border-b border-zinc-800">
                <tr>
                  <th className="p-4 text-left font-semibold text-zinc-400">Invoice #</th>
                  <th className="p-4 text-left font-semibold text-zinc-400">Customer</th>
                  <th className="p-4 text-left font-semibold text-zinc-400">Type</th>
                  <th className="p-4 text-left font-semibold text-zinc-400">Status</th>
                  <th className="p-4 text-left font-semibold text-zinc-400">Total</th>
                  <th className="p-4 text-left font-semibold text-zinc-400">Date</th>
                </tr>
              </thead>
              <tbody>
                {pageItems.map((inv) => (
                  <tr
                    key={inv.id}
                    className="border-t border-zinc-800/80 hover:bg-zinc-800/30 cursor-pointer transition text-zinc-300"
                    onClick={() => router.push(`/invoice/${inv.id}`)}
                  >
                    <td className={`p-4 font-bold text-white border-l-4 font-mono ${inv.type === 'sale' ? 'border-l-[#1E3A8A]' : 'border-l-[#C05621]'}`}>
                      {inv.invoice_number}
                    </td>
                    <td className="p-4 text-zinc-200">{inv.customers?.name}</td>
                    <td className="p-4"><TypeBadge type={inv.type} /></td>
                    <td className="p-4">
                    ...
                    <td className="p-4 font-mono font-bold text-white">{formatCurrency(inv.total, inv.currency)}</td>
                    <td className="p-4 text-zinc-400">{new Date(inv.created_at).toLocaleDateString()}</td>
                  </tr>
                ))}
              </tbody>
            </table>
